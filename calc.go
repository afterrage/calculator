...
L00P:
	...
	if len(numbers_a) == 2 {
		...
		if Anum1 < 1 || Anum1 > 10 || Anum2 < 1 || Anum2 > 10 {
			fmt.Println("Выдача паники, так как введенные цифры не от 1 до 10")
			goto L00P
		}
		...
		if len(txt_bez_vsego3) > 0 {
			fmt.Println("Выдача паники, так как были введены не подходящие знаки")
			goto L00P
		}
		...
		if txt_bez != correct_txt {
			fmt.Println("Выдача паники, так как формат математической операции не удовлетворяет заданию — два операнда и один оператор между ними (+, -, /, *)")
			goto L00P
		}
	...
	if len(numbers_r) == 2 {
		...
		if convert1 < 1 || convert1 > 10 || convert2 < 1 || convert2 > 10 {
			fmt.Println("Выдача паники, так как введенные римские цифры не от I до X")
			goto L00P
		}
		...
		if len(numbers_r) == 2 {
			...
			if znak == "-" {
				...
				if result < "1" {
					fmt.Println("Выдача паники, так как в римской системе нет отрицательных чисел и нуля")
					goto L00P
				}
			}
			if znak == "*" {
